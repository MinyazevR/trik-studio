...
{
	if (mBox2DRobots.contains(robot)) {
		delete mBox2DRobots[robot]; // clazy:exclude=detaching-member
	}

	mBox2DRobots[robot] = new Box2DRobot(this, robot, positionToBox2D(pos), angleToBox2D(angle));
	mLeftWheels[robot] = mBox2DRobots[robot]->getWheelAt(0); // clazy:exclude=detaching-member
	mRightWheels[robot] = mBox2DRobots[robot]->getWheelAt(1); // clazy:exclude=detaching-member
}
...
void Box2DPhysicsEngine::onRobotStartPositionChanged(QPointF newPos, model::RobotModel *robot)
{
	...
	mBox2DRobots[robot]->moveToPoint(positionToBox2D(newPos + robotCenter)); // clazy:exclude=detaching-member
}
...
void Box2DPhysicsEngine::onRobotStartAngleChanged(const qreal newAngle, model::RobotModel *robot)
{
	...
	mBox2DRobots[robot]->setRotation(angleToBox2D(newAngle)); // clazy:exclude=detaching-member
}

void Box2DPhysicsEngine::onMouseReleased(QPointF newPos, qreal newAngle)
{
	Box2DRobot *robot = mBox2DRobots.first(); // clazy:exclude=detaching-member
...
void Box2DPhysicsEngine::recalculateParameters(qreal timeInterval)
{
	...
	model::RobotModel * const robot = mRobots.first(); // clazy:exclude=detaching-member
	if (!mBox2DRobots[robot]) {
		return;
	}

	b2BodyId rBodyId = mBox2DRobots[robot]->getBodyId(); // clazy:exclude=detaching-member
	float secondsInterval = timeInterval / 1000.0f;

	if (mBox2DRobots[robot]->isStopping()){ // clazy:exclude=detaching-member
		mBox2DRobots[robot]->stop(); // clazy:exclude=detaching-member
	} else {
		...
		if (qAbs(speed1) + qAbs(speed2) < FLT_EPSILON) {
			mBox2DRobots[robot]->stop(); // clazy:exclude=detaching-member
			mLeftWheels[robot]->stop(); // clazy:exclude=detaching-member
			mRightWheels[robot]->stop(); // clazy:exclude=detaching-member
		}
		else {
			mLeftWheels[robot]->keepConstantSpeed(speed1); // clazy:exclude=detaching-member
			mRightWheels[robot]->keepConstantSpeed(speed2); // clazy:exclude=detaching-member
		}
	}
	...
	auto bodyId = mBox2DRobots[robot]->getBodyId(); // clazy:exclude=detaching-member
	...
	auto wheelBodyId = mBox2DRobots[robot]->getWheelAt(0)->getBodyId(); // clazy:exclude=detaching-member
	auto wheelBodyAngle = b2Rot_GetAngle(b2Body_GetRotation(wheelBodyId));
	auto wheelBodyPosition = b2Body_GetPosition(wheelBodyId);

	auto wheel1BodyId = mBox2DRobots[robot]->getWheelAt(1)->getBodyId(); // clazy:exclude=detaching-member
...
void Box2DPhysicsEngine::onItemDragged(graphicsUtils::AbstractItem *item)
{
	...
	if (auto wallItem = qobject_cast<items::WallItem *>(item)) {
		...
		return;
	} else if (auto solidItem = qobject_cast<items::SolidItem *>(item)) {
