...
void WorldModel::clear()
{
	while (!mWalls.isEmpty()) {
		removeWall(mWalls.last()); // clazy:exclude=detaching-member
	}

	while (!mSkittles.isEmpty()) {
		removeSkittle(mSkittles.last()); // clazy:exclude=detaching-member
	}

	while (!mBalls.isEmpty()) {
		removeBall(mBalls.last()); // clazy:exclude=detaching-member
	}

	while (!mColorFields.isEmpty()) {
		removeColorField(mColorFields.last()); // clazy:exclude=detaching-member
	}

	while (!mImageItems.isEmpty()) {
		removeImageItem(mImageItems.last()); // clazy:exclude=detaching-member
	}

	while (!mRegions.isEmpty()) {
		auto toRemove = mRegions.last(); // clazy:exclude=detaching-member
		mRegions.remove(toRemove->id());
		Q_EMIT itemRemoved(toRemove);
	}

	while (!mComments.isEmpty()) {
		auto toRemove = mComments.last(); // clazy:exclude=detaching-member
...
void WorldModel::clearRobotTrace()
{
	while (!mRobotTrace.isEmpty()) {
		auto const toRemove = mRobotTrace.first(); // clazy:exclude=detaching-member
