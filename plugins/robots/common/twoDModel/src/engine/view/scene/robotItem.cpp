...
void RobotItem::removeSensor(const kitBase::robotModel::PortInfo &port)
{
	...
	auto sensor = mSensors[port]; // clazy:exclude=detaching-member
...
{
	if (mSensors[port]) { // clazy:exclude=detaching-member
		mSensors[port]->setPos(mRobotModel.configuration().position(port)); // clazy:exclude=detaching-member
		Q_EMIT sensorUpdated(mSensors[port]); // clazy:exclude=detaching-member
	}
}
...
{
	if (mSensors[port]) { // clazy:exclude=detaching-member
		mSensors[port]->setRotation(mRobotModel.configuration().direction(port)); // clazy:exclude=detaching-member
		Q_EMIT sensorUpdated(mSensors[port]); // clazy:exclude=detaching-member
	}
}
