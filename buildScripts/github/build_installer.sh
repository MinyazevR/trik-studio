...
NEED_DEPLOY=$([[ "$GITHUB_REPOSITORY" == "MinyzevR/trik-studio" && "${PULLREQUESTNUMBER:-false}" == "false" ]] && echo true || echo false )
...
if $NEED_DEPLOY ; then
    $EXECUTOR bash -c "\
          INSTALLER_NAME=$(find installer -name "trik-studio*installer*" -quit) \
