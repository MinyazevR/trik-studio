...
if [[ -n "${QT_CONF_FILE+x}" ]]; then
    qmake -qtconf "$QT_CONF_FILE" -spec $QT_PREFIX/mkspecs/macx-clang -Wall PYTHON_VERSION=3.$TRIK_PYTHON3_VERSION_MINOR PYTHON_PATH=/usr CONFIG+=$CONFIG $QMAKE_EXTRA "$ROOT_DIR/$PROJECT.pro"
else
